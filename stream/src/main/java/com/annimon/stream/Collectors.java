...
public final class Collectors {
    ...
    /**
     * Returns a {@code Collector} that fills new {@code Collection}, provided by {@code
     * collectionSupplier}, with input elements.
     *
     ...
     *
     * <p>The returned {@code Collector} disallows {@code null}s and throws {@code
     ...
     *
     * <p>The returned {@code Collector} disallows {@code null}s and throws {@code
     * NullPointerException} if it is presented with a null value. If elements contain duplicates,
     ...
     *
     * <p>If the mapped keys contain duplicates, an {@code IllegalStateException} is thrown. Use
     * {@link #toMap(Function, Function, BinaryOperator)} to handle merging of the values.
     *
    ...
    public static <T, K, V> Collector<T, ?, Map<K, V>> toMap(
        ...
        return Collectors.<T, K, V, Map<K, V>>toMap(
                keyMapper, valueMapper, Collectors.<K, V>hashMapSupplier());
    }
     ...
     *
     * <p>If the mapped keys contain duplicates, an {@code IllegalStateException} is thrown. Use
     * {@link #toMap(Function, Function, BinaryOperator, Supplier)} to handle merging of the values.
     *
     * @param <T> the type of the input elements
     * @param <K> the result type of key mapping function
     * @param <V> the result type of value mapping function
     ...
     *
     * <p>The returned {@code Collector} disallows {@code null} keys and values. If the mapped keys
     * contain duplicates, an {@code IllegalStateException} is thrown, see {@link
     ...
     * Returns a {@code Collector} that fills new {@code Map} with input elements.
     *
     * <p>If the mapped keys contain duplicates, the value mapping function is applied to each equal
    ...
    public static <T, K, V> Collector<T, ?, Map<K, V>> toMap(
        ...
        return Collectors.<T, K, V, Map<K, V>>toMap(
                keyMapper, valueMapper, mergeFunction, Collectors.<K, V>hashMapSupplier());
    }
     ...
     * Returns a {@code Collector} that fills new unmodifiable {@code Map} with input elements.
     *
     * <p>The returned {@code Collector} disallows {@code null} keys and values.
     *
     * @param <T> the type of the input elements
     * @param <K> the result type of key mapping function
     * @param <V> the result type of value mapping function
     ...
     * @deprecated As of release 1.1.3, replaced by {@link
     *     #averagingDouble(com.annimon.stream.function.ToDoubleFunction)}
     * @return a {@code Collector}
     */
    @Deprecated
    @NotNull
    public static <T> Collector<T, ?, Double> averaging(
            @NotNull final Function<? super T, Double> mapper) {
    ...
    public static <T> Collector<T, ?, Double> averagingLong(
            @NotNull final ToLongFunction<? super T> mapper) {
        return averagingHelper(
                new BiConsumer<long[], T>() {
                    @Override
                    public void accept(long[] t, T u) {
    ...
    @NotNull
    private static <T> Collector<T, ?, Double> averagingHelper(
            @NotNull final BiConsumer<long[], T> accumulator) {
     ...
     * @return a {@code Collector}
     * @see #reducing(java.lang.Object, com.annimon.stream.function.Function,
     *     com.annimon.stream.function.BinaryOperator)
     */
    @NotNull
    public static <T> Collector<T, ?, T> reducing(
            @Nullable final T identity, @NotNull final BinaryOperator<T> op) {
        ...
        return new CollectorsImpl<T, A, R>(
                ...
                new BiConsumer<A, T>() {
                    ...
                    public void accept(A a, T t) {
                        if (predicate.test(t)) accumulator.accept(a, t);
                    }
                },
                downstream.finisher());
    }
    ...
    public static <T, A, IR, OR> Collector<T, A, OR> collectingAndThen(
        ...
        return new CollectorsImpl<T, A, OR>(
                c.supplier(), c.accumulator(), Function.Util.andThen(c.finisher(), finisher));
    }
     ...
     * @see #groupingBy(com.annimon.stream.function.Function, com.annimon.stream.function.Supplier,
    ...
    public static <T, K, A, D> Collector<T, ?, Map<K, D>> groupingBy(
        ...
        return Collectors.<T, K, D, A, Map<K, D>>groupingBy(
                classifier, Collectors.<K, D>hashMapSupplier(), downstream);
    }
     ...
     * The returned {@code Map} always contains mappings for both {@code false} and {@code true}
     * keys.
     *
     * @param <T> the type of the input elements
    ...
    public static <T, D, A> Collector<T, ?, Map<Boolean, D>> partitioningBy(
        ...
        return new CollectorsImpl<T, Tuple2<A>, Map<Boolean, D>>(
                new Supplier<Tuple2<A>>() {
                    ...
                    public Tuple2<A> get() {
                        return new Tuple2<A>(
                                downstream.supplier().get(), downstream.supplier().get());
                    }
                },
    ...
    /**
     * Returns a {@code Collector} that composites two collectors. Each element is processed by two
     * specified collectors, then their results are merged using the merge function into the final
     * result.
     *
     * @param <T> the type of the input elements
        ...
        return new CollectorsImpl<T, Map.Entry<A1, A2>, R>(
                ...
                new Function<Map.Entry<A1, A2>, R>() {
                    ...
                    public R apply(@NotNull Map.Entry<A1, A2> entry) {
                        return merger.apply(
                                finisher1.apply(entry.getKey()), finisher2.apply(entry.getValue()));
                    }
                });
    }
    ...
    private static IllegalStateException duplicateKeyException(
            Object key, Object old, Object value) {
        return new IllegalStateException(
                String.format(
                        "Duplicate key %s (attempted merging values %s and %s)", key, old, value));
    }

    private static <K, V> void mapMerge(
            @NotNull Map<K, V> map, K key, V value, @NotNull BinaryOperator<V> merger) {
